...
contract OnchainAuction is ReentrancyGuard, AdminControl {
    ...
    IRoyaltyEngine immutable royaltySupport;
    ...
    /// @param royaltySupportArg  The address of RoyaltyEngine
    /// @param durationForCheckArg  the Time period to check if nearing to end
    /// @param durationToIncreaseArg  the Time Extension of end time
    constructor(
        address platformAddressArg,
        uint16 platformFeePercentageArg,
        IRoyaltyEngine royaltySupportArg,
        uint64 durationForCheckArg,
        uint64 durationToIncreaseArg) {
        ...
        durationForCheck = durationForCheckArg;
        durationToIncrease = durationToIncreaseArg;

        platformAddress = payable(platformAddressArg);
        platformFeePercentage = platformFeePercentageArg;
        royaltySupport = royaltySupportArg;
    }
    ...
    /// @param durationForCheckArg Time period to check the nearest to end.
    /// @param durationToIncreaseArg  Time Extension of end time. 
    function updateValuesOfAutoTimeExtension(uint64 durationForCheckArg, uint64 durationToIncreaseArg)
    external adminRequired {
       emit AutoTimeExtensionUpdated(durationForCheck,durationForCheckArg,durationToIncrease,durationToIncreaseArg);
       durationForCheck = durationForCheckArg;
       durationToIncrease = durationToIncreaseArg;
    }
    ...
    {
        require(platformAddressArg != address(0), "invalid Platform Address");
        emit PlatformAddressUpdated(platformAddress,platformAddressArg);
        platformAddress = payable(platformAddressArg);
    }

    /// @notice Update the Platform Fee Percentage
    /// @param platformFeePercentageArg The Platform fee percentage
    function updatePlatformFeePercentage(uint16 platformFeePercentageArg)
        external
        adminRequired
    {
        require(
        ...
        emit PlatformFeePercentageUpdated(platformFeePercentage,platformFeePercentageArg);
        platformFeePercentage = platformFeePercentageArg;
