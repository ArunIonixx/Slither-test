...
/**
 * @dev RoyaltyPayoutEngine to lookup royalty configurations.The main purpose of this contract to getRoyalty
 ...
 */
contract RoyaltyEngineVersion1 is IRoyaltyEngine, AdminControlUpgradeable{
    ...
    constructor(uint256 maxBasisPoints) {
        require(
            ...
            "maxBasisPoints should not be equal or exceed than the value 10000");
    ...
    **/    
    ...
    returns(address payable[] memory recipients,uint256[] memory basisPoints) {
        if (tokenRoyaltyReceivers[collectionAddress][tokenId].length > 0){
            ...
            (recipients, basisPoints) = getRoyaltyStandardInfo(collectionAddress,tokenId);
            return (recipients, basisPoints);
        }
    }
    ...
    **/  
    function getRoyaltyStandardInfo(address collectionAddress, uint256 tokenId)
        ...
        returns (address payable[] memory recipients, uint256[] memory basisPoints){
            ...
            try IEIP2981(collectionAddress).royaltyInfo(tokenId, value) returns (address recipient, uint256 amount) {
                require(amount < value, "Invalid royalty amount");
                try IRoyaltySplitter(collectionAddress).getRecipients() returns (Recipient[] memory splitRecipients) {
                    ...
                    basisPoints[0] = (amount * 10000) / value;
                    return (recipients,basisPoints);
                }
            } catch { }
            ...
            if (
                ...
                returns (address payable creator) {
                     try ISuperRareRegistry(SuperRareContracts.SUPERRARE_REGISTRY).calculateRoyaltyFee(
                        collectionAddress, tokenId, value
            ...
            returns (IRaribleV2.Part[] memory royalties) {
                ...
                require(recipients.length == basisPoints.length, "Invalid royalty amount");
                return (recipients,basisPoints);
            } catch {}
            ...
            try IRaribleV1(collectionAddress).getFeeRecipients(tokenId) returns (
                ...
                recipients_ = IRaribleV1(collectionAddress).getFeeRecipients(tokenId);
            ...
            try IZoraOverride(collectionAddress).convertBidShares(collectionAddress, tokenId)
            ...
            try IArtBlocksOverride(collectionAddress).getRoyalties(collectionAddress, tokenId) returns (
            ...
            try IKODAV2Override(collectionAddress).getKODAV2RoyaltyInfo(collectionAddress, tokenId, value) returns (
                ...
                basisPoints[i] = (_amounts[i] * 10000) / value;
    ...
    **/
    ...
    {
        if (tokenRoyaltyReceivers[collectionAddress][tokenId].length > 0) {
            receivers = new address payable[](tokenRoyaltyReceivers[collectionAddress][tokenId].length);
            basisPoints = new uint256[](tokenRoyaltyReceivers[collectionAddress][tokenId].length);
            feeAmount = new uint256[](tokenRoyaltyReceivers[collectionAddress][tokenId].length);
            receivers = tokenRoyaltyReceivers[collectionAddress][tokenId];
            basisPoints = tokenRoyaltyBPS[collectionAddress][tokenId];
            for (uint256 i = 0; i < receivers.length;i++) {
                feeAmount[i] = (basisPoints[i] * amount) / 10000;
            }
            return (receivers, basisPoints, feeAmount);
        } else if (collectionRoyaltyReceivers[collectionAddress].length > 0) {
            receivers = new address payable[](tokenRoyaltyReceivers[collectionAddress][tokenId].length);
            basisPoints = new uint256[](collectionRoyaltyReceivers[collectionAddress].length);
            feeAmount = new uint256[](collectionRoyaltyReceivers[collectionAddress].length);
    ...
    **/
    ...
    /**
     * @notice Adds Collection addres as blacklist
     * @param collectionAddress contract address 
    **/
    function blacklistCollectionAddress(address collectionAddress)
        external
        override
        adminRequired
    {
        if (!blacklistedCollectionAddress.contains(collectionAddress)) {
            blacklistedCollectionAddress.add(collectionAddress);
            emit AddedBlacklistedAddress(collectionAddress, msg.sender);
        }
    }
    ...
    **/
    function blacklistWalletAddress(address walletAddress)
        external
        override
        adminRequired
    {
        if (!blacklistedWalletAddress.contains(walletAddress)) {
            blacklistedWalletAddress.add(walletAddress);
            emit AddedBlacklistedAddress(walletAddress, msg.sender);
        }
    }
    ...
    **/
    function revokeBlaclistedAddress(address commonAddress)
        external
        override
        adminRequired
    {
